...
{
    public partial class Frm_KHACH_HANG : Form
    {
        ...
        private void RELOAD_DATA_FROM_SQL()
        {
            ...
            vmk.MS_SQL_QUERY = "SELECT [MA_KH], (SELECT [TEN_NKH_NCC] FROM [DM_NKH_NCC] WHERE [MA_NKH_NCC] = KHACH_HANG.MA_NKH_NCC) AS [NKH_NCC], [TEN_KH], [DIA_CHI], [SDT], [FAX], [EMAIL], [MST], [GHI_CHU] FROM KHACH_HANG ORDER BY [MA_KH] ASC";
            ...
            if (DT.Rows.Count == 0) { dgv_ds_kh.DataSource = null; dgv_ds_kh.Columns.Clear(); return; }

            dgv_ds_kh.DataSource = DT;

            // CHỈNH SỬA TIÊU ĐỀ CÁC CỘT CHO ĐỐI TƯỢNG DATAGRIDVIEW

            dgv_ds_kh.Columns["MA_KH"].HeaderText = "MÃ KH";
            dgv_ds_kh.Columns["NKH_NCC"].HeaderText = "NHÓM KH";
            dgv_ds_kh.Columns["TEN_KH"].HeaderText = "TÊN KH";
            dgv_ds_kh.Columns["DIA_CHI"].HeaderText = "ĐỊA CHỈ";
            dgv_ds_kh.Columns["SDT"].HeaderText = "SĐT";
            dgv_ds_kh.Columns["FAX"].HeaderText = "FAX";
            dgv_ds_kh.Columns["EMAIL"].HeaderText = "EMAIL";
            dgv_ds_kh.Columns["MST"].HeaderText = "MÃ SỐ THUẾ";
            dgv_ds_kh.Columns["GHI_CHU"].HeaderText = "GHI CHÚ";
        }

        private void btn_xoa_Click(object sender, EventArgs e)
        {
			if (dgv_ds_kh.Rows.Count == 0 || dgv_ds_kh.SelectedRows.Count == 0){ return; }

            string ma_kh = dgv_ds_kh.SelectedRows[0].Cells["MA_KH"].Value.ToString().Trim();
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "DELETE FROM KHACH_HANG WHERE MA_KH = @MA_KH";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
        ...
        private void btn_them_Click(object sender, EventArgs e)
        {
            ...
            Frm_KHACH_HANG_CHI_TIET Frm = new Frm_KHACH_HANG_CHI_TIET();
            Frm.ID_DATA = "";
