...
namespace QUẢN_LÝ_SẢN_PHẨM
{
    public partial class Frm_KHACH_HANG_CHI_TIET : Form
    {
        ...
        private void Frm_KHACHHANG_CHI_TIET_Load(object sender, EventArgs e)
        {
        ...
        private void XEM_CSDL_THEO_ID(string ID_DATA)
        {
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "SELECT [MA_KH], [MA_NKH_NCC], [TEN_KH], [DIA_CHI], [SDT], [FAX], [EMAIL], [MST], [GHI_CHU] FROM KHACH_HANG WHERE (MA_KH = @MA_KH) ORDER BY MA_KH ASC";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
            vmk.MS_SQL_PARAMETERS.Rows.Add("@MA_KH", ID_DATA, SqlDbType.VarChar);
            ArrayList KQ = vmk.MS_SQL_SELECT();
            ...
            if (DT.Rows.Count != 0)
            {
                txt_ma_kh.Text = DT.Rows[0]["MA_KH"].ToString().Trim();
                cbo_ma_nkhncc.SelectedValue = DT.Rows[0]["MA_NKH_NCC"].ToString().Trim();
                txt_ten_kh.Text = DT.Rows[0]["TEN_KH"].ToString().Trim();
        ...
        private void btn_them_Click(object sender, EventArgs e)
        {
            ...
            string ma_kh = txt_ma_kh.Text.Trim().ToUpper();
            string ma_nkh_ncc = cbo_ma_nkhncc.SelectedValue.ToString();
            string ten_kh = txt_ten_kh.Text.Trim();
            ...
            if (ma_kh == "" || ma_nkh_ncc == "" || ten_kh == "" || dia_chi == "")
            {
            ...
            // KIỂM TRA MÃ NHÓM KHÁCH HÀNG

            if (ma_nkh_ncc != "" && CFuns.CHECK_CHARACTER(ma_nkh_ncc, "_", true, true, false) == false)
            {
                MessageBox.Show("MÃ NHÓM KHÁCH HÀNG CÓ CHỨA NHỮNG KÝ TỰ KHÔNG HỢP LỆ", "THÔNG BÁO");
                return;
            }

            // KIỂM TRA TÊN KHÁCH HÀNG

            if (ten_kh != "" && CFuns.CHECK_CHARACTER(ten_kh, " ", true, true, true) == false)
            {
                MessageBox.Show("TÊN KHÁCH HÀNG CÓ CHỨA NHỮNG KÝ TỰ KHÔNG HỢP LỆ", "THÔNG BÁO");
                return;
            }
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "INSERT INTO [KHACH_HANG] ([MA_KH], [MA_NKH_NCC], [TEN_KH], [DIA_CHI], [SDT], [FAX], [EMAIL], [MST], [GHI_CHU]) VALUES (@MA_KH, @MA_NKH_NCC, @TEN_KH, @DIA_CHI, @SDT, @FAX, @EMAIL, @MST, @GHI_CHU)";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
            ...
            vmk.MS_SQL_PARAMETERS.Rows.Add("@TEN_KH", ten_kh, SqlDbType.NVarChar);
        ...
        private void btn_capnhat_Click(object sender, EventArgs e)
        {
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "UPDATE KHACH_HANG SET [MA_NKH_NCC] = @MA_NKH_NCC, [TEN_KH] = @TEN_KH, [DIA_CHI] = @DIA_CHI, [SDT] = @SDT, [FAX] = @FAX, [EMAIL] = @EMAIL, [MST] = @MST, [GHI_CHU] = @GHI_CHU WHERE [MA_KH] = @MA_KH";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
            vmk.MS_SQL_PARAMETERS.Rows.Add("@MA_KH", ma_kh, SqlDbType.VarChar);
            vmk.MS_SQL_PARAMETERS.Rows.Add("@MA_NKH_NCC", ma_nkh_ncc, SqlDbType.VarChar);
